...
export default function Index() {
  ...
  // helper: add node programmatically (click on sidebar)
  ...
  function handleConnectClick(targetId) {
    ...
    // build new connection: ensure single incoming for target (remove existing incoming)
    setConnections(prev => {
      const withoutTo = prev.filter(c => c.to !== targetId);
      // remove outgoing from connectingFrom if you want single outgoing per source? Keep previous behavior: allow multiple outgoing
      // we remove any existing identical connection
  ...
  function handleSubmit() {
    ...
    // map to JSON per fn id -> { data: X }  (demo data)
